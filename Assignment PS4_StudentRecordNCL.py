...
input_file = open("C:/Users/parth/Desktop/inputPS4.txt")
for record in input_file.readlines():
    student = record.split(" / ")
    studentId = student[0]
    CGPA = float(student[1].rstrip("\n"))
    CGPAFrom = float(student[1].rstrip("\n"))
    CGPATo = float(student[1].rstrip("\n"))
...
hallOfFame(StudentHashRecords, 4.5)
...
def newCourseList(StudentHashRecords, CGPAFrom,CGPATo):
    ...
    output_file = open("outputPS4NCL.txt", "w+")
    output_file.write("---------- New Course Candidate ----------")
    output_file.write("\nInput: " + str(CGPAFrom ) )
    output_file.write(" Tod " + str(CGPATo ) )
    output_file.write("\nTotal eligible students: " + str(len(qualified_students)))
    output_file.write("\nQualified students:")
    for student in qualified_students:
        output_file.write("\n" + student[0] + " / " + str(student[1]))
...
newCourseList(StudentHashRecords, 3.5, 4.0)    
